...
AGGREGATED_DIR = BASE_DIR / "data" / "ncs_aggregated_latest"
...
def step1_decompress_raw_ncs():
    ...
    print("\n🚀 [1/3] Распаковка сырых NCS-бинарников (.bin) из PAK-файлов...")
...
def step2_convert_bin_to_raw_json():
    ...
    print("\n🚀 [2/3] Дамп всех .bin файлов в Сырые Необрезанные JSON...")
...
def step3_generate_useful_manifests():
    """Генерация только ТЕХ встроенных файлов, которые работают идеально"""
    print("\n🚀 [3/3] Генерация валидных служебных файлов (drops, parts_db, decoder)...")

    # 1. parts_database.json
    out_parts = AGGREGATED_DIR / "parts_database.json"
    cmd_parts = [str(BL4_BIN), "ncs", "extract", str(RAW_BIN_DIR), "-t", "manifest", "--json"]
    res_p = subprocess.run(cmd_parts, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
    if res_p.returncode == 0 and res_p.stdout.strip():
        with open(out_parts, "w", encoding="utf-8") as f:
            f.write(res_p.stdout)

    # 2. serial_decoder.json
    out_dec = AGGREGATED_DIR / "serial_decoder.json"
    cmd_dec = [str(BL4_BIN), "ncs", "extract", str(RAW_BIN_DIR), "-t", "decoder", "--json"]
    res_d = subprocess.run(cmd_dec, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
    if res_d.returncode == 0 and res_d.stdout.strip():
        with open(out_dec, "w", encoding="utf-8") as f:
            f.write(res_d.stdout)

    # 3. drops.json
    drops_file = AGGREGATED_DIR / "drops.json"
    cmd_drops = [
        str(BL4_BIN), "drops", "generate",
        str(RAW_BIN_DIR),
        "-o", str(drops_file),
        "--manifest-dir", str(AGGREGATED_DIR)
    ]
    subprocess.run(cmd_drops, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
    
    print("✅ Служебные файлы успешно сохранены!")
...
def main():
    ...
    step3_generate_useful_manifests()
